...
class Ollama:
    ...
    def __init__(
        ...
        This constructor sets up the model, tools, prompt formatting, and conversation state.
    ...
    async def invoke(self, user_query: str = '', state: Messages | None = None):
        """
            ...
            if self.mcp_client is not None:
                async with self.mcp_client:
                    for tool_call in tool_calls:
                        ...
                        self.state['messages'].append(self.create_message(
                            role='tool',
                            content=tool_response.content[0].text)  # type: ignore[union-attr]
                        )
        except ValueError:
            pass
        return self.state
